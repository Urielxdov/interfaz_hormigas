import { IApiInventarioRepository, InventarioItemDTO, CreateInventarioDTO } from '@hormigas/application'
...
export class ApiInventarioRepositoryImpl implements IApiInventarioRepository {
  ...
  async crear(dto: CreateInventarioDTO): Promise<InventarioItemDTO> {
    const row = await this.http.post<ServerInventarioDTO>('/api/inventario/crear', dto)
    return toDTO(row)
  }
}
